...
if PY3:
    long = int
    INTEGER_TYPES = (int,)
    INTEGER_OR_BYTES = (int, Bytes)
else:
    INTEGER_TYPES = (int, long)
    INTEGER_OR_BYTES = (int, long, Bytes)
...
class _BaseNet(_IPAddrBase):
    ...
    def __eq__(self, other):
        try:
            ...
            if isinstance(other, _BaseIP):
                return (
                    self._version == other._version and self._ip == other._ip
                )
    ...
    def hostmask(self):
        ...
        if x is None:
            x = IPAddress(
                int(self.netmask) ^ self._ALL_ONES, version=self._version
            )
    ...
    def address_exclude(self, other):
        ...
        if other not in self:
            raise ValueError(
                "%s not contained in %s" % (str(other), str(self))
            )
        if other == self:
            return []
        ...
        other = IPNetwork(
            "%s/%s" % (str(other.network), str(other.prefixlen)),
    ...
    def _prefix_from_prefix_string(self, prefixlen_str):
        ...
        try:
            ...
            raise NetmaskValueError(
                "%s is not a valid prefix length" % prefixlen_str
            )
        return prefixlen
    ...
    def iter_subnets(self, prefixlen_diff=1, new_prefix=None):
        ...
        Args:
            ...
            new_prefi$x: The desired new prefix length. This must be a
        ...
        first = IPNetwork(
            "%s/%s"
            % (str(self.network), str(self._prefixlen + prefixlen_diff)),
        ...
        while True:
            ...
            current = IPNetwork(
                "%s/%s" % (str(new_addr), str(new_prefixlen)),
                version=self._version,
            )
    ...
    def supernet(self, prefixlen_diff=1, new_prefix=None):
        ...
        return IPNetwork(
            "%s/%s"
            % (str(self.network), str(self.prefixlen - prefixlen_diff)),
            version=self._version,
        )
...
class IPv4Network(_BaseV4, _BaseNet):
    ...
    def __init__(self, address, strict=False):
        ...
        if isinstance(address, INTEGER_OR_BYTES):
...
class _BaseV6(object):
    ...
    def _ip_int_from_string(self, ip_str):
        ...
        try:
            skip_index, = [
                i for i in range(1, len(parts) - 1) if not parts[i]
            ] or [None]
        except ValueError:
    ...
    def _string_from_ip_int(self, ip_int=None):
        ...
        for x in range(0, 32, 4):
            y = x + 4
            hextets.append("%x" % int(hex_str[x:y], 16))
