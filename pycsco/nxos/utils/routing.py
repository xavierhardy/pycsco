...
def _parse_vrf_static_routes(run_config):
    ...
    for i in range(1, len(lines)):
        ...
        if line.startswith("ip route"):
            ...
            prefix = fields[2]
            next_hop = fields[3]
            vrf_route_dict[_route_id_tag(prefix, next_hop)] = _pop_route_dict(
                fields
            )
...
def _get_default_vrf_static_routes(device, prefix, next_hop):
    default_static_output = device.show(
        'show run | inc "^ip route"', text=True
    )
    default_static_dict = xmltodict.parse(default_static_output[1])
    default_static_run_config = default_static_dict["ins_api"]["outputs"][
        "output"
    ]["body"]

    if default_static_run_config:
        default_static_routes = _parse_default_static_routes(
...
def get_static_routes(device, vrf, prefix, next_hop):
    ...
    """
    if vrf == "default":
        static_routes = _get_default_vrf_static_routes(
            device, prefix, next_hop
        )
    else:
