...
def set_aaa_server_global_key(encrypt_type, key, server_type):
    if not encrypt_type:
        encrypt_type = ""
    return "{0}-server key {1} {2}".format(server_type, encrypt_type, key)
...
def get_aaa_server_info(device, server_type):
    ...
    response = device.show("show {0}-server".format(server_type), text=True)
...
def config_aaa_server(params, server_type):
    ...
    if global_key:
        cmds.append(
            set_aaa_server_global_key(encrypt_type, global_key, server_type)
        )
...
def get_aaa_host_info(device, server_type, address):
    ...
    response = device.show(
        'sh run | inc "{0}-server host {1}"'.format(server_type, address),
        text=True,
    )
    response_dict = xmltodict.parse(response[1])
    response_text = response_dict["ins_api"]["outputs"]["output"]["body"]
...
def config_aaa_host(server_type, address, params, clear=False):
    ...
    if key:
        cmds.append(
            "{0}-server host {1} key {2} {3}".format(
                server_type, address, enc_type, key
            )
        )
