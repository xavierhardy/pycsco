...
__all__ = [
    ...
    "create_dir",
    "feature_enabled",
    "get_active_vpc_peer_link",
    "get_interface_running_config",
    "get_existing_portchannel_to_vpc_mappings",
    "get_facts",
    "get_vrf",
    "get_feature_list",
    "get_hsrp_group",
    "get_interface_mode",
    "get_hsrp_groups_on_interfaces",
    "vlan_range_to_list",
    "switch_files_list",
    "get_interface",
    ...
    "get_neighbors",
    "get_portchannel",
    "get_portchannel_list",
    "get_portchannel_vpc_config",
    "get_switchport",
    "get_system_mtu",
    "get_udld_global",
    "get_udld_interface",
    "get_vlan",
    "get_vpc",
    "get_vpc_running_config",
    "get_vrf_list",
    "peer_link_exists",
    "interface_is_portchannel",
    "is_default",
    "is_interface_copper",
    "delete_dir",
    "interface_range_to_list",
]
...
def get_vlan_info(device):
    ...
    try:
        ...
        temp = {
            "vlan_id": str(resource_table.get("vlanshowbr-vlanid", None)),
            "name": str(resource_table.get("vlanshowbr-vlanname", None)),
            "admin_state": str(
                resource_table.get("vlanshowbr-shutstate", None)
            ),
...
def get_interface(device, intf):
    ...
    if i:
        ...
        if intf_type == "ethernet":
            interface["admin_state"] = str(
                i.get("admin_state", "unable_to_get_n3K_api_bug")
            )
            ...
            interface["admin_state"] = str(
                attributes.get("admin_state", "error")
            )
            interface["description"] = str(
                attributes.get(
                    "description", "unable_to_read_ + " " nxapi_bug"
                )
            )
...
def get_commands_if_mode_change(proposed, existing, group, mode):
    ...
    for interface, values in members_dict.items():
        if interface in proposed_members and (
            interface not in members_to_remove_list
        ):
...
def config_portchannel(proposed, mode, group):
    ...
    if min_links:
        ...
        commands.append(command)
        commands.append(
            CONFIG_ARGS.get("min_links").format(min_links=min_links)
        )
...
def get_vrf_description(device, vrf):
    ...
    """
    command = (
        "show run section vrf | begin {0}".format(vrf)
        + " | include description"
    )
...
def get_commands_to_config_vpc(vpc, domain, existing):
    ...
    if pkl_src and pkl_dest:
        pkl_command = "peer-keepalive destination {pkl_dest}".format(
            **vpc
        ...
        if pkl_src and pkl_dest:
            pkl_command = "peer-keepalive destination {0}".format(
                pkl_dest
            ) + " source {1} vrf {2}".format(pkl_src, pkl_vrf)
            commands.append(pkl_command)

    if not vpc.get("auto_recovery", None):
        vpc["ar"] = "no"
    else:
        vpc["ar"] = ""

    if not vpc.get("peer_gw", None):
        vpc["pg"] = "no"
    else:
        vpc["pg"] = ""
...
def get_vpc(device):
    ...
    if vpc_dict:
        ...
        auto_recovery = get_autorecovery(
            str(vpc_dict["vpc-auto-recovery-status"])
        )
...
def get_active_vpc_peer_link(device):
    ...
    try:
        ...
        data_dict = xmltodict.parse(data[1])
        peer_link = str(
            data_dict["ins_api"]["outputs"]["output"]["body"].get(
                "TABLE_peerlink"
            )["ROW_peerlink"]["peerlink-ifindex"]
        )
    except (KeyError, AttributeError):
        peer_link = None
...
def feature_enabled(device, feature):
    ...
    try:
        ...
        if error is None:
            features = table["body"].get("TABLE_cfcFeatureCtrlTable")[
...
def nested_cmd_list_to_string(commands):
    cmds = ""
    if commands:
        cmds = " ".join(" ; ".join(each) + " ;" for each in commands if each)
    return cmds
...
def get_facts(device):
    ...
    fan_list = []
    try:
        for each in resource_table["fandetails"].get("TABLE_faninfo")[
            "ROW_faninfo"
        ]:
            temp = {}
...
def get_interface_detail(device, interface):
    ...
    if intf:
        ...
        temp["load_interval1_rx"] = str(
            each.get("eth_load_interval1_rx", None)
        )
        temp["inrate1_bits"] = str(each.get("eth_inrate1_bits", None))
        temp["inrate1_pkts"] = str(each.get("eth_inrate1_pkts", None))
        temp["load_interval1_tx"] = str(
            each.get("eth_load_interval1_tx", None)
        )
...
def get_mtu(device, interface):
    ...
    if intf_dict:
        resource["mtu"] = str(
            intf_dict.get(
                "eth_mtu", intf_dict.get("svi_mtu", "unreadable_via_api")
            )
        )
...
def get_commands_config_mtu(delta, interface):
    ...
    """
    CONFIG_ARGS = {"mtu": "mtu {mtu}", "sysmtu": "system jumbomtu {sysmtu}"}
