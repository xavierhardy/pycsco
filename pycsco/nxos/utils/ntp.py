...
def set_ntp_trusted_key(trusted_key):
    return "ntp trusted-keys {0}".format(trusted_key)
...
def enable_ntp_source(src_srcint, src_addr_int):
    return "ntp {0} {1}".format(src_srcint, src_addr_int)


def disable_ntp_trusted_key(trusted_key):
    return "no ntp trusted-keys {0}".format(trusted_key)
...
def get_ntp_auth_key(device, key_id):
    ntp_auth_cfg_response = device.show(
        'show run | inc "ntp authentication-key {0}"'.format(key_id), text=True
    )
    ntp_auth_run_cfg = xmltodict.parse(ntp_auth_cfg_response[1])["ins_api"][
        "outputs"
    ]["output"]["body"]
...
def get_ntp_auth(device):
    response = device.show("show ntp authentication-status")
    response_dict = xmltodict.parse(response[1])
    ntp_auth_str = response_dict["ins_api"]["outputs"]["output"]["body"][
        "authentication"
    ]
...
def get_ntp_log(device):
    response = device.show("show ntp logging")
    response_dict = xmltodict.parse(response[1])
    ntp_log_str = response_dict["ins_api"]["outputs"]["output"]["body"][
        "loggingstatus"
    ]
...
def get_ntp_master(device):
    response = device.show('show run | inc "ntp master"', text=True)
    response_dict = xmltodict.parse(response[1])
...
def get_ntp_trusted_key(device):
    ...
    response = device.show('show run | inc "ntp trusted-key"', text=True)
    response_dict = xmltodict.parse(response[1])
...
def get_ntp_source(device):
    ...
    response = device.show('show run | inc "ntp source"', text=True)
    response_dict = xmltodict.parse(response[1])
...
def config_ntp(delta, existing):
    ...
    if serv_peer:
        ntp_cmds.append(
            set_ntp_server_peer(serv_peer, address, prefer, key_id, vrf_name)
        )
    if source_addr_int:
        ...
        ntp_cmds.append(enable_ntp_source(source_type, source_addr_int))
...
def remove_ntp_auth_key(
    ...
    if authentication == "on":
        ...
        auth_remove_cmds.append("ntp authenticate")
...
def disable_ntp_server_peer(serv_peer, address):
    ...
    """
    return "no ntp {0} {1}".format(serv_peer, address)
...
def disable_ntp_source(src_srcint, src_addr_int):
    ...
    """
    return "no ntp {0} {1}".format(src_srcint, src_addr_int)
...
def get_ntp_existing(device, address, serv_peer):
    ...
    existing["source_type"], existing["source_addr_int"] = get_ntp_source(
        device
    )
